...
> `inquiries`는 회원(users)과 FK 연관관계가 없는 독립 테이블이다 — 비회원도 문의를 남길 수 있어(F-44) 작성자를 `user_id`가 아니라 회신용 `email` 문자열로만 받는다.
...
> 이 엔드포인트는 MVP2의 F-22(회원 차단/해제/강제탈퇴)와 동일한 API다 — 회원 관리 기능이 MVP2에서 차단/차단해제와 함께 재정리되면서 권한 매트릭스 쪽 번호도 F-22로 옮겨졌다.
...
> 멘토 목록/상세/리뷰 조회는 GUEST가 아니라 **USER**(로그인 필요)다 — `SecurityConfig` 주석의 "구독 여부와 상관없이 공개"는 로그인 여부가 아니라 구독 여부에 대한 이야기였고, 실제 `.authenticated()` 규칙이 팀의 의도(2026-08-26 확인)와 일치한다.
...
> F-ID는 팀 Notion 요구사항정의서/기능명세서/API명세서와 동일한 **F-01~F-51** 체계로 통일했다(기존에는 이 문서만 별도로 F-01~F-88 번호를 매겨서 Notion과 어긋나 있었음, 2026-08-26 정리). 상위 F-ID 하나가 여러 엔드포인트로 구현된 경우(예: 회원가입/로그인 하위의 재발급·로그아웃, 프로필 수정 하위의 이메일·비밀번호 수정)는 같은 F-ID를 반복 표기했다. **Notion 요구사항정의서에 아예 F-ID가 없는 엔드포인트(전체 회원 목록 `GET /api/users`, 특정 유저 질문/답변 목록, 공개 프로필/프로필 이미지 개별 수정, 헬스체크 등)는 이 표에서 제외했다** — F-ID 없이 문서에 남겨두지 않기로 했고, 필요하면 Notion에 먼저 정식으로 채번한 뒤 이 문서에 반영한다.
...
> 주: F-06(질문 수정/삭제)의 실제 코드는 서로 다르다 — **수정은 작성자 본인만**(`validateAuthor`), **삭제는 작성자 또는 관리자**(`validateAuthorOrAdmin`). 답변(F-08)도 동일한 비대칭 구조.
...
> ⚠️ **F-21, 아직 미해결 — 팀의 남은 마지막 보안 이슈**: `SecurityConfig`상 `permitAll()`로 공개되는 게 맞지만, 서비스가 반환하는 `UserResponse`에 `email`/`blocked`/`emailVerified`까지 그대로 담겨 있다. API 명세서상 정의된 안전한 응답({id,name,profileImageUrl,followerCount,followingCount,isFollowing})보다 훨씬 많이 노출 중 — 비로그인 상태로 `userId`만 순차 대입하면 전체 회원 이메일을 긁어올 수 있다. **팀 결정: 문서/제출 준비를 먼저 마치고 가장 마지막에 코드로 수정.**
...
> ✅ **F-41(멘토 목록/상세/리뷰) — 팀 확인 완료, 코드 그대로 유지.** `SecurityConfig` 주석은 "구독 여부와 상관없이 공개"라고 적혀 있는데, 이건 **로그인 여부**가 아니라 **구독 여부**에 대한 이야기였다 — 로그인은 해야 하지만 구독을 했든 안 했든 볼 수 있다는 뜻으로, 실제 `.authenticated()` 규칙과 정확히 일치한다(2026-08-26, 박준성 확인). API명세서/화면설계 문서의 표기(과거엔 "GUEST/전체"로 잘못 적혀 있었음)를 코드에 맞게 정정 완료.
> F-33: 서비스에 `mentorId==본인` 체크만 있고 `Role==MENTOR` 체크는 없다. 승인 안 된 일반 USER도 이론상 자기 자신의 멘토 프로필을 만들거나 수정할 수 있다(멘토 목록 노출 조건을 못 채우면 검색엔 안 잡히므로 실질적 위험은 크지 않음).
...
> 요금제 목록/단건 조회: `SecurityConfig`에 `/api/v1/mentors/**` 규칙 자체가 없어서 전부 `anyRequest().authenticated()`로 떨어진다 — **비로그인 상태로는 요금제(가격) 조회조차 불가능**하다. 멘토 목록/상세와 마찬가지로 "로그인 필요"가 팀의 의도된 정책이므로 별도 조치 없음.
> 요금제 등록/수정/삭제: `mentorId==본인` 체크만 있고 `Role==MENTOR` 체크는 없음(F-33과 동일한 패턴).
...
> ⚠️ 아티클 수정: 본인 검증에 실패하거나 글이 없을 때 서비스가 `IllegalArgumentException`을 던지는데, `GlobalExceptionHandler`에서 **400(잘못된 요청)으로 처리된다 — 원래는 403/404여야 할 상황이 400으로 나간다.** 같은 컨트롤러의 삭제는 `CustomException(ACCESS_DENIED)`를 써서 정상적으로 403이 난다 — 같은 파일 안에서 수정/삭제가 서로 다른 방식을 쓰고 있다(API 명세서에도 반영함).
...
> ✅ **수정 완료.** F-51 댓글 작성/삭제는 원래 로그인 토큰이 아니라 클라이언트가 보내는 `X-USER-ID` 헤더를 그대로 신뢰했다 — 그 헤더 값을 아무 숫자로나 바꾸면 다른 사용자 명의로 댓글을 작성하거나 남의 댓글을 삭제할 수 있는 인증 우회 취약점이었다. 삭제 시 권한 위반이 `IllegalStateException`으로 던져져 `GlobalExceptionHandler`가 처리하지 못해 403 대신 500 에러가 나던 것도 함께 있었다.
> 조치(완료): `@RequestHeader("X-USER-ID")` → `@AuthenticationPrincipal`(JWT 기반)로 교체, 삭제 시 예외를 `CustomException(ErrorCode.ACCESS_DENIED)`(403)/`RESOURCE_NOT_FOUND`(404)로 정리. 위 표는 수정 후 기준이다.
...
> ⚠️ 구독 해지 예약 / 구독별 결제 이력: 본인 소유가 아니거나 구독이 존재하지 않을 때 `IllegalArgumentException`을 던져서 **403/404가 아니라 400**으로 나간다(아티클 수정과 같은 종류의 문제, API 명세서에도 반영함).
...
> 파일 다운로드: 멘토 아티클(구독자 전용 글)에 첨부된 파일도 질문 첨부와 같은 방식으로 열리는지는 끝까지 확인 못했다. 비구독자가 첨부 ID만 알면 유료 글의 첨부파일을 우회 다운로드할 수 있는지 **실제로 테스트해보는 걸 권장**한다.
...
> 정산 계좌 등록은 MENTOR/ADMIN 둘 다 허용, 정산 내역 조회는 MENTOR만 허용(ADMIN도 막힘) — 같은 정산 도메인 안에서 ADMIN 취급이 엔드포인트마다 다르다. 의도적인지 팀 확인 권장.
